...
export default {
  ...
  methods: {
    ...
    async cargarCorreos() {
      try {
        const response = await axios.get(`http://localhost:8088/api/usuarios/correos`);  // Quitar empresaId
    ...
    async cargarUsuarios() {
      try {
        ...
        const responseAlarma = await axios.get(`http://localhost:8088/api/usuarios/alarmaCorreo`);  // Quitar empresaId
        this.usuariosAlarma = responseAlarma.data;

        // Cargar usuarios para errores
        const responseError = await axios.get(`http://localhost:8088/api/usuarios/alarmaError`);  // Quitar empresaId
    ...
    async agregarUsuario() {
      ...
      try {
        if (this.pestanaActiva === 0) {
          ...
          const response = await axios.post('http://localhost:8088/api/usuarios/alarmaCorreo/add', { email: nuevoUsuario.email });
          ...
          const response = await axios.post('http://localhost:8088/api/usuarios/alarmaError/add', { email: nuevoUsuario.email });
          if (response.status === 200 || response.status === 201) {
            Swal.fire("Usuario agregado correctamente", "", "success").then(() => {
              this.$router.go(0);
            });
    ...
    async eliminarUsuario(id, tipo) {
      const url = tipo === 'alarma'
          ? `http://localhost:8088/api/usuarios/alarmaCorreo/${id}/remove`
          : `http://localhost:8088/api/usuarios/alarmaError/${id}/remove`;
