...
export default {
  ...
  methods: {
    fetchTarifas() {
      axios.get("https://app.evolgreen.com:8088/api/fees")
          .then(response => {
    ...
    fetchConnectors() {
      axios.get("https://app.evolgreen.com:8088/api/connectors")
          .then(response => {
    ...
    assignPlan() {
      if (this.selectedTarifa && this.selectedConnectors.length > 0) {
        ...
        const requests = this.selectedConnectors.map(conector => {
          return axios.patch(`https://app.evolgreen.com:8088/api/connectors/${conector.id}/assign-fee`, null, {
            params: {
