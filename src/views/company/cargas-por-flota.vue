<template>
  <Layout>
    <PageHeader title="Cargas por Flota" pageTitle="Compañía" />
...
export default {
    ...
    resultQuery() {
      ...
      if (this.searchQuery) {
        ...
        filteredData = filteredData.filter((data) => {
          return (
              data.flota.toLowerCase().includes(search) ||
  ...
  methods: {
    ...
    exportToCSV() {
      const headers = [
        "Estación de Carga","Cargador", "Conector", "Inicio de Carga","Fin Carga", "Flota",  "ID Cargador",   "Energía", "Tiempo"
      ];
      const rows = this.filteredData.map(item => [
        item.estacionDeCarga, item.cargador, item.conector, item.inicioCarga, item.finCarga, item.flota,  item.idCargador,  item.energia, item.tiempo
      ]);
    ...
    exportToExcel() {
      const ws = XLSX.utils.json_to_sheet(this.filteredData, {
        header: ["estacion de carga","cargador", "conector","inicioCarga",  "finCarga", "flota", "idCargador", "energia", "tiempo"]
      });
