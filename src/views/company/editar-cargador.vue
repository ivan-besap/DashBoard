...
export default {
  ...
  methods: {
    async loadCharger() {
      ...
      try {
        const response = await axios.get(`http://localhost:8088/api/chargers/${chargerId}`);
    ...
    async updateCharger() {
      try {
        ...
        const response = await axios.put(`http://localhost:8088/api/companies/current/chargers/${this.charger.id}`, dataParaEnviar);
