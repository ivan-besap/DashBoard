  ...
  export default {
    ...
    methods: {
      async loadChargingStation() {
        try {
          const response = await axios.get(`http://localhost:8080/api/chargingStations/${this.stationId}`);
      ...
      async updateChargingStation() {
        try {
          await axios.put(`http://localhost:8080/api/companies/current/chargingStations/${this.stationId}`, this.chargingStation);
