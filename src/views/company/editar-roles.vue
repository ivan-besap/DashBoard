...
export default {
  ...
  methods: {
    async fetchRole() {
      ...
      try {
        const response = await axios.get(`http://localhost:8088/api/roles/${roleId}`);
    ...
    async fetchPermisos() {
      try {
        const response = await axios.get("http://localhost:8088/api/permissions");
    ...
    async updateRole() {
      try {
        ...
        await axios.put(`http://localhost:8088/api/roles/${this.role.id}`, roleData);
        this.successmsg();
      } catch (error) {
