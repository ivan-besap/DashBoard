        ...
        <div class="table-responsive table-card">
          <table class="table align-middle table-nowrap" id="customerTable">
            ...
            <tr v-for="(dat, index) in resultQuery" :key="index">
              <td>{{ dat.name }}</td>
              <td>
                <BButton style="padding: 5px 10px;" variant="light" class="waves-effect waves-light"  v-if="permisos.includes(68)">
                  <router-link class="nav-link menu-link" :to="`/company/editar-fabricante/${dat.id}`">
                    <i class="mdi mdi-pencil"></i>
                  </router-link>
                </BButton>
                <BButton style="padding: 5px 10px;  margin-left: 10px" variant="light" class="waves-effect waves-light" @click="confirm(dat.id)"  v-if="permisos.includes(70)">
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            ...
            const response = await axios.patch(`https://app.evolgreen.com/api/manufacturers/${manufacturerId}`);
