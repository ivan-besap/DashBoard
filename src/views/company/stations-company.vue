...
export default {
  ...
  methods: {
    ...
    async cambiarActivoEstacion(id, estadoTerminal) {
      try {
        const response = await axios.patch(`https://app.evolgreen.com:8088/api/chargingStationsStatus/change-active-status`, null, {
          params: {
            id: id,
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            ...
            const response = await axios.patch(`https://app.evolgreen.com:8088/api/companies/current/chargingStations/${stationId}/delete`);
            if (response.status === 200 || response.status === 201){
